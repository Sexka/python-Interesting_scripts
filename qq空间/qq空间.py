...
def startSpider():
    driver = webdriver.Chrome(executable_path=chrome_driver) #这个是chormedriver的地址
    ...
    driver.find_element_by_id('u').send_keys('916149179')  #这里填写你的QQ号
    driver.find_element_by_id('p').clear()
    driver.find_element_by_id('p').send_keys('if(love==TRUE);')  #这里填写你的QQ密码
 
    driver.find_element_by_id('login_button').click()
    time.sleep(2)
 
    #设置爬取内容保存路径
    f = open('shuoshuo.txt','w',encoding='utf-8')
    ...
    #--------------获得好友列表   注意下面的链接
    driver.get('https://user.qzone.qq.com/proxy/domain/r.qzone.qq.com/cgi-bin/tfriend/friend_hat_get.cgi?hat_seed=1&uin=916149179fupdate=1&g_tk='+str(gtk)+'&qzonetoken='+str(g_qzonetoken)+'&g_tk='+str(gtk))
    ...
    for key in numList.keys():
        ...
        if QQnum == "783279200":  #根据qq号查找指定好友说说
            count = 1
            begin = 0
            while tag==1 :
